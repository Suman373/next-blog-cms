...
export const postCategoryRouter = router({
    assignCategories: publicProcedure.input(z.object({
        ...
        if(input.categoryIds.length > 0){
            const values = input.categoryIds.map(catId=> ({
                postId: input.postId,
                catId,
            }));
    ...
    getPostCategories: publicProcedure.input(z.object({postId: z.number()})).query(async ({input})=> {
        const result = await db.select({category: categories}).from(postCategories).innerJoin(categories, eq(postCategories.id, categories.id)).where(eq(postCategories.id, input.postId));
